...
void
stan_elf64_process_segments (STAN_CORE *k)
{
  Elf64_Ehdr*  elf_hdr;
  Elf64_Phdr*  elf_seg;
  int          n_seg;
...
void
stan_elf64_process_symtab (STAN_CORE *k, Elf64_Shdr* s)
{
  char *sname;
  int n_entries;
  int  i;
...
stan_elf64_process_sections (STAN_CORE *k)
{
  ...
  elf_hdr = (Elf64_Ehdr *) k->code;
  shdr = (Elf64_Shdr *)(k->code + elf_hdr->e_shoff);
...
stan_elf64_init (STAN_CORE *k)
{
  ...
  // Extract Segments
  stan_elf64_process_segments (k);
  stan_table_sort (k->seg);

  // Extract Sections
  stan_elf64_process_sections (k);
...
int  
stan_elf64_process (STAN_CORE *k)
{
  // Process Sections 
